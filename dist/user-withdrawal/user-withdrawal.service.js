            ...
            try {
                ...
                if (feeData) {
                    ...
                    const minFee = isStablecoin
                        ? (chainId === "1" ? 3 : chainId === "TRON" ? 5 : 0.5)
                        : (MIN_ADMIN_FEE[chainId] || 0.001);
                    if (adminFeeAmount > 0 && adminFeeAmount < minFee) {
                        console.log(`[Withdrawal Fee] ⏭️ SKIPPING dust admin fee: ${adminFeeAmount.toFixed(token.decimal)} ${token.symbol} ` +
        ...
        try {
            const payload = {
                ...
                withdrawal: {
                    ...
                    adminFee: withdrawal.adminFee || null,
                    ...
                    transactionType: withdrawal.transactionType || null,
                    fiatCurrency: withdrawal.fiatCurrency || null,
                    fiatAmount: withdrawal.fiatAmount || null,
                    cryptoAmount: withdrawal.cryptoAmount || null,
                    pricePerCoin: withdrawal.pricePerCoin || null,
                },
                status: withdrawal.status,
            };
