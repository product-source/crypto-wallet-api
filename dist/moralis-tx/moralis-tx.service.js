        ...
        try {
            ...
            for (const wallet of selectedPaymentLinks) {
                ...
                if (currentRetry > MAX_WITHDRAW_RETRIES) {
                    console.error(`[EVM Withdraw] ❌ Max retries (${MAX_WITHDRAW_RETRIES}) exceeded for link ${wallet?._id}. ` +
                        `Marking as WITHDRAW_FAILED to stop retry loop.`);
                    await this.paymentLinkModel.updateOne({ _id: wallet._id }, {
                        $set: {
                ...
                await this.paymentLinkModel.updateOne({ _id: wallet._id }, { $inc: { withdrawRetryCount: 1 } });
    ...
    async withdrawTronPaymentFromLinks() {
        try {
            ...
            for (const link of partialPaymentLinks) {
                ...
                if (currentRetry > MAX_WITHDRAW_RETRIES) {
                    console.error(`[TRON Withdraw] ❌ Max retries (${MAX_WITHDRAW_RETRIES}) exceeded for link ${link?._id}. ` +
                        `Marking as WITHDRAW_FAILED to stop retry loop.`);
                    await this.paymentLinkModel.updateOne({ _id: link._id }, {
                        $set: {
                            withdrawStatus: 'WITHDRAW_FAILED',
                            withdrawRetryCount: currentRetry,
                        },
                ...
                await this.paymentLinkModel.updateOne({ _id: link._id }, { $inc: { withdrawRetryCount: 1 } });
            ...
            for (const link of completedLinks) {
                try {
                    ...
                    if (currentRetry > MAX_RECLAIM_RETRIES) {
                        console.error(`[Reclaim] ❌ Max retries (${MAX_RECLAIM_RETRIES}) exceeded for link ${link?._id}. ` +
                            `Marking as reclaimed to stop retry loop.`);
                        await this.paymentLinkModel.updateOne({ _id: link._id }, { $set: { fundsReclaimed: true, reclaimRetryCount: currentRetry } });
                        continue;
                    }
                    await this.paymentLinkModel.updateOne({ _id: link._id }, { $inc: { reclaimRetryCount: 1 } });
            ...
            for (const token of trc20Tokens) {
                ...
                if (BigInt(tokenBalance) > BigInt(0) && tokenAddr) {
                    ...
                    if (gasResult.trxNeeded > 0 && humanAmount < gasResult.trxNeeded * 0.15) {
                        console.log(`[Reclaim TRON] ⏭️ Not profitable: ~$${humanAmount.toFixed(2)} < gas ~${gasResult.trxNeeded} TRX ($${(gasResult.trxNeeded * 0.15).toFixed(2)}). Skip.`);
                        continue;
                    }
                    if (gasResult.funded) {
                        try {
    ...
    async reclaimEvmFunds(tempAddress, privateKey, chainId, link) {
        ...
        try {
                        ...
                        if (humanBalance < 0.5) {
                            ...
                            if (humanBalance < 1 && gasCostEth > 0.001) {
                                ...
                                if (BigInt(nativeBalance) < gasCostWei * BigInt(2)) {
                                    ...
                                    const fundRes = await (0, evm_helper_1.evmNativeTokenTransferToPaymentLinks)(chainId, deficit, account.address);
                                    if (fundRes.status) {
            ...
            if (BigInt(nativeBalance) > minReclaim) {
                ...
                console.log(`[Reclaim EVM] Sweeping ${web3.utils.fromWei(reclaimWei.toString(), "ether")} native → ${adminAddress} ` +
