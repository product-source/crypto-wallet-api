...
let AdminService = class AdminService {
    ...
    async updateFiatWallet(dto, user) {
        try {
            const { FiatEvmAdminWallet, FiatTronAdminWallet, FiatbtcAdminWallet } = dto;
            if (user && !user?.isAdmin) {
                throw new common_1.ForbiddenException("Unauthorized access, Admin privilege required");
            }
            const admin = await this.adminModel.findOne();
            ...
            if (FiatEvmAdminWallet) {
                if (!(await (0, evm_helper_1.isValidEVMAddress)(FiatEvmAdminWallet))) {
                    return new common_1.NotFoundException("Invalid EVM wallet address");
                }
                else {
            ...
            if (FiatTronAdminWallet) {
                if (await (0, tron_helper_1.isValidTronAddress)(FiatTronAdminWallet)) {
            ...
            if (FiatbtcAdminWallet) {
                if ((0, bitcoin_address_validation_1.validate)(FiatbtcAdminWallet)) {
