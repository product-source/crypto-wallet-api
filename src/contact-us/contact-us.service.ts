...
import { Pricing, PricingDocument } from "src/pricing/schema/pricing.schema";

@Injectable()
export class ContactUsService {
  constructor(
    @InjectModel(ContactUs.name)
    private readonly contactModel: Model<ContactUsDocument>,
  ...
  async addContact(dto: AddContactDto, file: any) {
    try {
  ...
  async viewContact(id) {
    try {
      ...
      if (!user) {
        throw new NotFoundException("Invalid Conatct Id");
      }
      let pricingData;
      if (user.pricingId) {
  ...
  async updateConatact(dto: UpdateContactDto) {
    try {
      ...
      //only one time reply is allow

      if (contact?.status === "REPLIED") {
        throw new BadRequestException("Can not reply again");
      }
      try {
        // Sending mail
        if (adminReply) {
